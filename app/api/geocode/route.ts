...
function searchKnownPlaces(query: string): NormalizedResult[] {
...
function queryVariants(query: string): string[] {
  ...
  if (!/\b(india|nepal|bangladesh|pakistan|bhutan|sri lanka)\b/i.test(query)) {
    variants.push(`${query}, India`);
  }

  if (!/\btelangana\b/i.test(query)) {
    variants.push(`${query}, Telangana, India`);
  }

  if (/\bwarangal\b|\bhanamkonda\b/i.test(lower)) {
    variants.push(`${query}, Warangal, Telangana, India`);
  }

  if (
    /\btemple\b|\buniversity\b|\bcollege\b|\binstitute\b|\bcampus\b|\bfort\b|\blake\b|\bhospital\b/i.test(
      lower
    )
  ) {
    variants.push(`${query}, Warangal, Telangana, India`);
  ...
  if (/\bsr\s*university\b|\bsru\b/i.test(query)) {
    variants.push(
      "SR University, Ananthasagar, Hasanparthy, Warangal, Telangana, India"
    );
    variants.push("SR University, Warangal, Telangana, India");
  }

  if (/bhadrakali/i.test(query)) {
    variants.push(query.replace(/bhadrakali/gi, "Badrakali"));
    variants.push(
      `${query.replace(
        /bhadrakali/gi,
        "Badrakali"
