...
function buildQueryVariants(query: string): string[] {
  ...
  // Adding geographic context greatly improves POI / landmark matching.
  ...
  // Warangal/Hanamkonda is a common local context for this project.
  if (
  ...
  // Handle common spelling variation for Bhadrakali/Badrakali.
...
export async function GET(request: Request) {
  ...
  try {
    // Search multiple increasingly contextual variants. This is especially
    // useful for landmarks, temples, campuses and other micro-level POIs.
    for (const variant of variants) {
      try {
        const matches = await searchNominatim(variant);
        results.push(...matches);

        // Once we have strong matches, do not keep making requests.
    ...
    // Fallback provider for POIs that are not indexed/returned by Nominatim.
