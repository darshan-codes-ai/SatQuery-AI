...
type StatisticsRow = {
  ...
  outputs?: { ndvi?: OutputStats; ndwi?: OutputStats; ndbi?: OutputStats };
};
...
type CatalogFeature = {
  id?: string;
  properties?: { datetime?: string; "eo:cloud_cover"?: number | string };
};
...
function normalizeSelection(value: unknown): SelectionGeometry | undefined {
  ...
  if (input.type === "Point" && validCoordinatePair(input.coordinates)) {
...
function getAnalysisGeometry(coordinates: Coordinates, selection?: SelectionGeometry) {
  if (selection?.type === "Polygon") {
    return { type: "Polygon" as const, coordinates: selection.coordinates };
  }

  const point: [number, number] = selection?.type === "Point"
...
async function getAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error("Sentinel Hub credentials are missing. Check .env.local.");
  }
...
async function findSentinelScenes(accessToken: string, geometry: SelectionGeometry): Promise<CatalogFeature[]> {
  ...
  features.sort((a, b) => {
    ...
    return new Date(b.properties?.datetime ?? 0).getTime() - new Date(a.properties?.datetime ?? 0).getTime();
  });
...
function evaluatePixel(sample) {
  ...
  const ndvi = ndviDenominator !== 0 ? (sample.B08 - sample.B04) / ndviDenominator : 0;
  const ndwi = ndwiDenominator !== 0 ? (sample.B03 - sample.B08) / ndwiDenominator : 0;
  const ndbi = ndbiDenominator !== 0 ? (sample.B11 - sample.B08) / ndbiDenominator : 0;
  return { ndvi: [ndvi], ndwi: [ndwi], ndbi: [ndbi], dataMask: [1] };
}
`;
  ...
  const response = await fetch(SENTINEL_STATISTICS_URL, {
    ...
    body: JSON.stringify({
      input: {
        ...
        data: [{ type: "sentinel-2-l2a", dataFilter: { maxCloudCoverage: 100, mosaickingOrder: "leastCC" } }],
      },
      aggregation: {
...
function extractObservation(statistics: StatisticsResponse) {
  ...
  for (const row of statistics.data) {
    ...
    if (Number.isFinite(ndvi) && Number.isFinite(ndwi) && Number.isFinite(ndbi)) {
      return { row, ndvi, ndwi, ndbi, ndviStats, ndwiStats, ndbiStats, statistics };
    }
  }
...
async function findUsableObservation(accessToken: string, geometry: SelectionGeometry, scenes: CatalogFeature[]) {
  for (const scene of scenes.slice(0, 12)) {
    try {
      const statistics = await getStatisticsForScene(accessToken, geometry, scene);
      const observation = extractObservation(statistics);
      if (observation) return { ...observation, scene };
    } catch (error) {
...
function createInterpretation(ndvi: number, ndwi: number, ndbi: number, coverage: number) {
  const vegetation = vegetationInterpretation(ndvi);
  const water = waterInterpretation(ndwi);
  const builtUp = builtUpInterpretation(ndbi);

  let overall: string;
  if (ndbi >= 0.25 && ndvi < 0.35) {
    overall = "The selected area appears predominantly built-up, with limited to moderate vegetation activity.";
  } else if (ndvi >= 0.55 && ndbi < 0.1) {
    overall = "The selected area appears predominantly vegetated, with relatively limited built-up surface signal.";
  } else if (ndwi >= 0.25) {
    overall = "The selected area has a notable water-related signal, with the surrounding surface characteristics varying by location.";
  } else {
    overall = "The selected area shows a mixed spectral profile across vegetation, water and built-up surfaces.";
  }

  const confidenceText = coverage >= 80
    ? "High confidence based on strong valid-pixel coverage."
    : coverage >= 50
      ? "Moderate confidence because some pixels were unavailable or masked."
      : "Lower confidence because valid satellite-pixel coverage is limited.";
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof query !== "string" || !query.trim()) {
      return NextResponse.json({ success: false, error: "A valid query is required." }, { status: 400 });
    }

    if (!coordinates || typeof coordinates.lat !== "number" || typeof coordinates.lng !== "number" || !Number.isFinite(coordinates.lat) || !Number.isFinite(coordinates.lng)) {
    ...
    const geometry = getAnalysisGeometry(coordinates, selection);
    const analysisType = detectAnalysisType(query);
    const accessToken = await getAccessToken();
    const scenes = await findSentinelScenes(accessToken, geometry);

    if (!scenes.length) {
      throw new Error("No Sentinel-2 scene was found for the selected area during the last year. Try selecting a different area.");
    }

    const result = await findUsableObservation(accessToken, geometry, scenes);
    if (!result) {
      throw new Error("Sentinel-2 scenes were found, but none returned valid NDVI, NDWI and NDBI statistics for the selected area. Try a nearby area or a larger selection.");
    }

    const ndvi = toNumber(result.ndvi);
    const ndwi = toNumber(result.ndwi);
    const ndbi = toNumber(result.ndbi);
    const sampleCount = toNumber(result.ndviStats?.sampleCount);
    const noDataCount = toNumber(result.ndviStats?.noDataCount);
    const geometryPixelCount = toNumber(result.statistics.geometryPixelCount);
    ...
    const confidence = Math.max(0, Math.min(1, coverage / 100));
    const interpretation = createInterpretation(ndvi, ndwi, ndbi, coverage);
    ...
    switch (analysisType) {
      case "ndvi":
        primaryValue = ndvi;
        baseResponse = `The selected area has a mean Sentinel-2 NDVI of ${ndvi.toFixed(2)}. Higher NDVI values generally indicate stronger vegetation activity.`;
        break;
      case "ndwi":
        primaryValue = ndwi;
        baseResponse = `The selected area has a mean Sentinel-2 NDWI of ${ndwi.toFixed(2)}. Higher NDWI values generally indicate stronger surface-water signals.`;
        break;
      case "ndbi":
        primaryValue = ndbi;
        baseResponse = `The selected area has a mean Sentinel-2 NDBI of ${ndbi.toFixed(2)}. Higher NDBI values can indicate relatively built-up surfaces.`;
        break;
      case "change":
        baseResponse = `This is a temporal-change query. Reliable change detection requires comparable observations from multiple dates.`;
        break;
      default:
        baseResponse = `The selected Sentinel-2 area was analyzed successfully. Mean NDVI is ${ndvi.toFixed(2)}, mean NDWI is ${ndwi.toFixed(2)}, and mean NDBI is ${ndbi.toFixed(2)}.`;
        break;
    }
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        acquisitionDate: result.scene.properties?.datetime ?? null,
        cloudCoverage: toNumber(result.scene.properties?.["eo:cloud_cover"], 0),
        processingResolution: "20m",
        processing: "Sentinel Hub Catalog + Statistical API",
        dataQuality: { sampleCount, noDataCount, geometryPixelCount, validPixels },
      },
    });
  } catch (error) {
    console.error("SatQuery analysis error:", error);
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Satellite analysis failed." },
      { status: 500 }
    );
  }
}
