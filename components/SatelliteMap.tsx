...
export default function SatelliteMap({
  ...
  useEffect(() => {
    ...
    if (!map) return;
    if (selectionMode === "polygon") map.doubleClickZoom.disable();
  ...
  useEffect(() => {
    ...
    map.on("click", (event) => {
      ...
      if (mode === "point") {
        ...
        setSelectionData({ type: "Point", coordinates: [lng, lat] });
        return;
      }
      ...
      if (mode === "rectangle") {
        ...
        emitCoordinates((startLng + lng) / 2, (startLat + lat) / 2);
        setSelectionData({ type: "Polygon", coordinates: [polygon] });
        rectangleStartRef.current = null;
        return;
      }
      ...
      if (polygonPointsRef.current.length === 1) emitCoordinates(lng, lat);
    });
    ...
    helpers.__satqueryFinishPolygon = () => {
      ...
      emitCoordinates(sumLng / points.length, sumLat / points.length);
      setSelectionData({ type: "Polygon", coordinates: [ring] });
      polygonPointsRef.current = [];
      setPolygonCount(0);
    };
    ...
    helpers.__satqueryClearSelection = () => {
      ...
      onSelectionChangeRef.current?.({ type: "Point", coordinates: [0, 0] });
    };

    helpers.__satqueryGoToLocation = (lng: number, lat: number) => {
      map.flyTo({ center: [lng, lat], zoom: 17, duration: 1600, essential: true });
      emitCoordinates(lng, lat);
      createPointMarker(lng, lat);
      setSelectionData({ type: "Point", coordinates: [lng, lat] });
      rectangleStartRef.current = null;
  ...
  useEffect(() => {
    ...
    if (existing) {
      existing.updateImage({ url: evidenceUrl, coordinates: evidenceBounds });
      return;
    }
    ...
    map.addLayer(
      ...
      map.getLayer(SELECTION_FILL) ? SELECTION_FILL : undefined
    );
  ...
  useEffect(() => {
    ...
    const existing = map.getSource(sourceId) as maplibregl.ImageSource | undefined;
    if (existing) {
      existing.updateImage({ url: changeHeatmapUrl, coordinates: changeHeatmapBounds });
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!spectralUrl || !spectralBounds || evidenceUrl) {
      if (map.getLayer(SPECTRAL_LAYER)) map.removeLayer(SPECTRAL_LAYER);
      if (map.getSource(SPECTRAL_SOURCE)) map.removeSource(SPECTRAL_SOURCE);
      return;
    }

    const existing = map.getSource(SPECTRAL_SOURCE) as maplibregl.ImageSource | undefined;
    if (existing) {
      existing.updateImage({ url: spectralUrl, coordinates: spectralBounds });
      return;
    }
  ...
  }, [spectralUrl, spectralBounds, evidenceUrl]);
  ...
  const showSpectralLayer = async () => {
    ...
    if (!bounds) {
      setSpectralError("Please make a valid map selection first.");
      return;
    }

    setIsSpectralLoading(true);
    setSpectralError("");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data?.error || `${spectralIndex.toUpperCase()} layer could not be generated.`);
      }
      ...
      setSpectralBounds(bounds);
    } catch (error) {
      console.error("SatQuery spectral layer error:", error);
      setSpectralError(error instanceof Error ? error.message : "Spectral layer could not be generated.");
    } finally {
      setIsSpectralLoading(false);
    }
  };

  const hideSpectralLayer = () => {
    setSpectralError("");
    setSpectralBounds(null);
    setSpectralUrl((previous) => {
      if (previous) URL.revokeObjectURL(previous);
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 w-[min(520px,calc(100%-2rem))]">
        <form onSubmit={handleSearch} className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/80 backdrop-blur-xl p-2 shadow-2xl">
          ...
          />
          <button type="submit" disabled={!searchQuery.trim() || isSearching} className="rounded-lg bg-cyan-400 px-3 py-2 text-[10px] font-semibold text-black transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-40">
        ...
        {(searchResults.length > 0 || searchError) && (
          <div className="mt-2 overflow-hidden rounded-xl border border-white/10 bg-[#07111c]/95 backdrop-blur-xl shadow-2xl">
            ...
            {searchResults.map((result, index) => (
              <button key={`${result.lat}-${result.lon}-${index}`} type="button" onClick={() => chooseSearchResult(result)} className="block w-full border-b border-white/5 px-4 py-3 text-left text-xs text-gray-300 last:border-b-0 hover:bg-white/5 hover:text-white">
                {result.display_name}
              </button>
            ))}
          </div>
      ...
      <div className="absolute top-4 left-4 z-10 rounded-xl border border-white/10 bg-black/75 backdrop-blur-xl px-4 py-3 pointer-events-none">
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /><span className="text-xs font-semibold text-white">MAP ONLINE</span></div>
        <p className="text-[10px] text-gray-400 mt-1">Global geospatial workspace</p>
      </div>

      <div className="absolute top-4 right-24 z-10 rounded-xl border border-white/10 bg-black/75 backdrop-blur-xl p-2">
        <div className="text-[9px] uppercase tracking-widest text-gray-500 px-2 pb-2">Select area</div>
        <div className="flex gap-1">
          {([["point", "Point"], ["rectangle", "Rectangle"], ["polygon", "Polygon"]] as [SelectionMode, string][]).map(([mode, label]) => (
            <button key={mode} type="button" onClick={() => setSelectionMode(mode)} className={`px-2.5 py-1.5 rounded-lg text-[10px] transition ${selectionMode === mode ? "bg-cyan-400 text-black" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="absolute bottom-4 right-4 z-10 flex items-center gap-2">
        {selectionMode === "polygon" && polygonCount >= 3 && (
          <button type="button" onClick={finishPolygon} className="rounded-lg border border-cyan-400/30 bg-black/80 px-3 py-2 text-[10px] text-cyan-300 hover:bg-cyan-400/10">
            Finish Polygon ({polygonCount})
          </button>
        )}
        {hasSelection && (
          <button type="button" onClick={clearSelection} className="rounded-lg border border-white/10 bg-black/80 px-3 py-2 text-[10px] text-gray-400 hover:bg-white/5">
            Clear
          </button>
        )}
      </div>
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none">
        <div className="w-12 h-12 rounded-full border border-cyan-400/70 flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.9)]" /></div>
      </div>
      ...
      {changeHeatmapUrl && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 rounded-xl border border-amber-300/20 bg-black/85 backdrop-blur-xl px-4 py-3 shadow-xl">
          <div className="text-[9px] uppercase tracking-widest text-gray-400 mb-2">Change Heatmap</div>
          <div className="flex items-center gap-4 text-[9px] text-gray-400">
            <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-red-500" />Decrease</div>
            <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-gray-300" />Little / no change</div>
            <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-green-400" />Increase</div>
          </div>
        </div>
      )}
      ...
      {evidenceUrl && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 rounded-xl border border-white/10 bg-black/80 backdrop-blur-xl px-4 py-3 shadow-xl">
          <div className="text-[9px] uppercase tracking-widest text-gray-400 mb-2">{evidenceIndex.toUpperCase()} Evidence</div>
          <div className="text-[9px] text-gray-400">Visual spectral evidence overlay</div>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 z-10 rounded-xl border border-white/10 bg-black/75 backdrop-blur-xl px-4 py-3 pointer-events-none">
        <div className="text-[9px] text-gray-400 uppercase tracking-widest">Selected Coordinates</div>
        <div className="mt-1 font-mono text-xs text-cyan-300">{coordinates.lat.toFixed(4)}° N · {coordinates.lng.toFixed(4)}° E</div>
      </div>
